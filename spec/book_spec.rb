...
describe Book do
  describe "#initialize" do
    it "creates a new Book object with correct instance variables" do
      book = Book.new({genre: 'Fiction', author: 'J.K. Rowling', source: 'Library', label: 'Bloomsbury', publish_date: '2022-06-26', publisher: 'Bloomsbury', cover_state: 'good'})
  ...
  describe '#can_be_archived?' do
    context 'when cover state is good and the item is not 10+ years old' do
      it 'returns false' do
        book = Book.new({genre: 'Fiction', author: 'J.K. Rowling', source: 'Library', label: 'Harry Potter and the Philosopher\'s Stone', publish_date: '2022-06-26', publisher: 'Bloomsbury', cover_state: 'good'})
    ...
    context 'when item is 10+ years old and cover state is good' do
      it 'returns true' do
        book = Book.new({genre: 'Fiction', author: 'J.K. Rowling', source: 'Borrowed', label: 'Harry Potter and the Philosopher\'s Stone', publish_date: '1997-06-26', publisher: 'Bloomsbury', cover_state: 'good'})
        expect(book.can_be_archived?).to eq(true)
      end
    end
  end
